...
export default function OfficerDashboard() {
  ...
  const [officerId, setOfficerId] = useState(
    paramId || localStorage.getItem("officerId") || "",
  );
  ...
  const requestGPS = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        ...
        toast.error(
          "Location access denied. You must allow location to use dashboard.",
        );
        setGpsLoading(false);
      },
  ...
  useEffect(() => {
    if (paramId) {
      ...
      setIsLoggedIn(true);
      if (!paramId)
        navigate(`/officer/${localStorage.getItem("officerId")}`, {
          replace: true,
        });
    }
      ...
      <div className="min-h-screen bg-[#0b1120] flex items-center justify-center p-4 text-white">
        <div className="bg-[#1E293B] p-8 rounded-2xl w-full max-w-md border border-white/10 shadow-xl">
          ...
          <form onSubmit={handleLogin} className="flex flex-col gap-4">
            ...
            <button
              type="submit"
              className="bg-cyan-600 hover:bg-cyan-500 text-white font-bold py-3 rounded-xl transition"
            >
              Enter Portal
            </button>
          </form>
      ...
      <div className="min-h-screen bg-[#0b1120] flex items-center justify-center p-4 text-white">
        <div className="bg-[#1E293B] p-8 rounded-2xl w-full max-w-md border border-white/10 shadow-xl text-center">
          ...
          <p className="text-gray-400 mb-2">
            CivicCall needs your location to assign complaints to the nearest
            officer.
          </p>
          <p className="text-red-400 text-sm mb-8 bg-red-500/10 border border-red-500/20 rounded-lg px-4 py-3">
            ⚠️ You cannot access the dashboard without enabling location. This
            ensures complaints reach the right officer.
          </p>
          <button
            onClick={requestGPS}
            disabled={gpsLoading}
          ...
          <p className="text-gray-600 text-xs mt-4">
            Your location is only used for complaint assignment and updated
            every 5 minutes while on duty.
          </p>
        </div>
      </div>
    );
  ...
  const resolvedToday = complaints.filter(
    (c) =>
      c.status === "resolved" &&
      new Date(c.resolvedAt || c.time).toDateString() === today,
  ).length;
    ...
    <div className="min-h-screen bg-[#0b1120] text-gray-300 pb-12">
      <div className="bg-[#1E293B] border-b border-white/5 sticky top-0 z-10 px-4 py-4 flex justify-between items-center shadow-md">
        <h1 className="font-bold text-white">
          CivicCall <span className="text-cyan-400">Officer</span>
        </h1>
                  ...
                  <div className="flex flex-col items-end gap-1">
                    ...
                    {c.urgencyScore !== undefined && (
                      <span className="text-[10px] text-gray-400">
                        AI:{" "}
                        <span
                          className={`font-bold ${c.urgencyScore >= 8 ? "text-red-400" : c.urgencyScore >= 6 ? "text-orange-400" : "text-yellow-400"}`}
                        >
