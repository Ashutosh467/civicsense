...
export default function Signup() {
    ...
    return (
        <div className="min-h-screen bg-[#0b1120] flex flex-col justify-center items-center p-6 relative overflow-hidden">
            <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-blue-600/10 blur-[100px] rounded-full pointer-events-none" />
            ...
            <div className="w-full max-w-md bg-white/5 border border-white/10 backdrop-blur-xl p-8 rounded-3xl shadow-2xl relative z-10">
                ...
                <form onSubmit={handleSignup} className="space-y-4">
                    ...
                    <button
                        ...
                        className="w-full bg-cyan-500 hover:bg-cyan-400 text-slate-900 font-bold py-3 px-4 rounded-xl transition flex items-center justify-center gap-2 shadow-lg shadow-cyan-500/20 mt-6"
                    >
                        {isSubmitting ? <Loader2 className="w-5 h-5 animate-spin" /> : "Sign Up Securely"}
                    </button>
                </form>

                <div className="mt-8 text-center text-sm text-gray-400">
                    Already have an account? <Link to="/login" className="text-cyan-400 hover:text-cyan-300 font-medium transition">Sign in instead</Link>
                </div>
            </div>
        </div>
    );
